...
本项目 fork 自 [run-bigpig/jcp](https://github.com/run-bigpig/jcp)，在上游版本基础上进行了以下改进：

### ✨ 新增功能

- **Web 服务器模式** - 新增完整的 HTTP API 服务，支持浏览器访问和 Docker 部署
- **龙虎榜数据支持（Web 端）** - 为 Web 模式添加龙虎榜数据查询 API (`/api/longhubang/list`, `/api/longhubang/detail`)
- **AI 连接测试（Web 端）** - 为 Web 模式实现 AI 配置连通性测试接口 (`/api/ai/test`)

### 🔧 修复与优化

- **静态资源修复** - 修复 Web 模式下前端静态资源 404 和 MIME 类型错误
- **Web 端兼容性** - 移除 Web 端设置中的"软件更新"选项卡（该功能仅适用于桌面端自动更新）
- **前端类型定义** - 完善 TypeScript 类型定义，添加完整的龙虎榜数据类型

### 📦 部署支持

- **Docker 支持** - 添加多阶段 Dockerfile，支持容器化部署
- **GitHub Actions** - 配置自动构建和发布工作流
- **静态资源嵌入** - 使用 Go embed 将前端资源嵌入到二进制文件中
