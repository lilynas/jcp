...
// 取消会议 - 双模式支持
const cancelMeetingApi = async (stockCode: string) => {
  if (isWailsEnv()) {
    const app = await getWailsApp();
    return app?.CancelMeeting(stockCode);
  }
  // Web 模式暂不支持取消会议
};
...
export const AgentRoom: React.FC<AgentRoomProps> = ({ session, onSessionUpdate }) => {
  ...
  const cancelMeeting = (stockCode: string) => {
    // 调用后端取消 API
    cancelMeetingApi(stockCode).catch((err: any) => {
  ...
  useEffect(() => {
    ...
    getWailsRuntime().then(runtime => {
      if (!runtime) return;
      cleanup = runtime.EventsOn('strategy:changed', () => {
    ...
    return () => {
      getWailsRuntime().then(runtime => {
        if (runtime) runtime.EventsOff('strategy:changed');
      });
      if (cleanup) cleanup();
    };
  }, []);
